...
var options = (0, _setting.getting)();
/**
 * 图片滚动加载组件，当图片滚动进入浏览器的显示区域时会出发显示
 * @param renderMode 渲染模式。['all'|'none'], all表示父组件任何渲染都会导致img直接渲染，none表示父组件发生任何渲染都不会引起内部渲染，默认为none。设置为node可以明显提升渲染效率
 * @param onOff: 图片延迟加载的开关，默认会使用全局的onOff配置参数
 * @param loadSrc: 图片未滚入显示区域时异步加载图片地址
...
var Img = (0, _scrollOver2.default)()(function (_React$Component) {
    ...
    function _class() {
        ...
        var _this$props = _this.props,
            src = _this$props.src,
            actSrc = _this$props.actSrc,
            onOff = _this$props.onOff,
            className = _this$props.className;

        _this.state = {
            className: className,
            src: onOff ? false : actSrc
        };
