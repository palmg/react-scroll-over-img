...
var environment = (0, _environment.get)();
 ...
 */
var registerFLow = function registerFLow(src, img, callback) {
    //向流水线队尾部增加对象
    flowList.push({ src: src, img: img, callback: callback });
    isStop && function () {
 ...
 * @param src
 */
resultHandle = function resultHandle(cb, src) {
    cb({ suc: 'NONE' !== src, src: src });
    loadOneStream();
},
...
loadImg = function loadImg(item) {
    ...
    img.onerror = img.onabort = function () {
        return resultHandle(callback, 'NONE');
    };
    if (img.complete) {
        resultHandle(callback, src);
    } else {
        img.onload = function () {
            return resultHandle(callback, src);
        };
    }
};
